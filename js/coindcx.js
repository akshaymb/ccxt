...
module.exports = class coindcx extends Exchange {
    describe () {
        return this.deepExtend (super.describe (), {
            ...
            'urls': {
                ...
                'api': {
                    'exchange' : 'https://api.coindcx.com/exchange/v1',
                    'market' : 'https://public.coindcx.com/market_data'
    ...
    async fetchMarkets (params = {}) {
        const response = await this.exchangeGetMarketsDetails (params);
        // 
        //  [
        //     {
        //       "coindcx_name": "SNMBTC",
    ...
    async fetchOrderBook (symbol, limit = undefined, params = {}) {
        ...
        const response = await this.marketGetOrderbook (this.extend (request, params));
        // {
        //     "bids":{
        ...
        //     }
        // }
        const bids = Object.keys(response['bids']).map(function (key) {
            return [key, response['bids'][key]];
        });
        const reverseAsks = Object.keys(response['asks']).map(function (key) {
            return [key, response['asks'][key]];
        });
        let asks = [];
        for (let i = reverseAsks.length-1; i>=0; i--) {
            asks.push(reverseAsks[i])
        }
        response['asks'] = asks
        response['bids'] = bids
